...
async def search_elastic_search_v2(req: SearchIndexV2Request):
    ...
    try:
        ...
        result = search_across_indices(elastic_search_indices or current_index, query, sort, remaining_size, current_skip, highlight)
        process_results(result)
        previous_month = datetime.datetime.strptime(current_month, '%Y-%m') - datetime.timedelta(days=1)
        while remaining_size > 0 and previous_month >= datetime.datetime(2024, 4, 1):
            ...
            result = search_across_indices(current_index, query, sort, remaining_size, current_skip, highlight)
            process_results(result)
